...
class RRTPlanner:
    ...
    def sample_state(self):
        ...
        x = np.random.randint(low=0, high=len(self.occ_grid)-1)
        y = np.random.randint(low=0, high=len(self.occ_grid[0]-1))
    ...
    def steer_towards(self, s_nearest, s_rand, max_radius):
        ...
        if dist <= max_radius:
            ...
            r = dist / max_radius
            x = int(s_nearest.x + (s_rand.x - s_nearest.x) / r)
            y = int(s_nearest.y + (s_rand.y - s_nearest.y) / r)
    ...
    def path_is_obstacle_free(self, s_from, s_to):
        ...
        for i in range(max_checks):
            ...
            x = s_from.x + int((s_to.x - s_from.x) * float(i)/max_checks)
            y = s_from.y + int((s_to.y - s_from.y) * float(i)/max_checks)
            if not (self.occ_grid[y-5:y+5, x-5:x+5] == 0).all():
                return False
...
if __name__ == "__main__":
    ...
    # dest_state = State(600, 80, None)
    dest_state = State(200, 600, None)
