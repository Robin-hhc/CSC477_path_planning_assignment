...
class AStarPlanner:
    ...
    def heuristic(self, curr_state, dest_state):
        h = sqrt(np.square(curr_state.x - dest_state.x) +
                 np.square(curr_state.y - dest_state.y))
        return h
    ...
    def plan(self, start_state, dest_state):
        ...
        cost_to_come = float("inf") * np.ones((world.shape[0], world.shape[1]))
        cost_to_come[start_state.x, start_state.y] = 0
        priority = float("inf") * np.ones((world.shape[0], world.shape[1]))
        priority[start_state.x, start_state.y] = self.heuristic(start_state,
                                                                dest_state)
        ...
        while Q:
            ...
            for ns in self.get_neighboring_states(s):
                ...
                transition_distance = sqrt(
                    (ns.x - s.x) ** 2 + (ns.y - s.y) ** 2)
                alternative_dist_to_come_to_ns = cost_to_come[
                                                     s.x, s.y] + transition_distance
                alternative_priority = alternative_dist_to_come_to_ns + self.heuristic(
                    ns, dest_state)
                ...
                if (ns not in Q) or (
                        alternative_dist_to_come_to_ns < cost_to_come[
                    ns.x, ns.y]):
                    Q[ns] = alternative_priority
                    cost_to_come[ns.x, ns.y] = alternative_dist_to_come_to_ns
                    priority[ns.x, ns.y] = alternative_priority
                    parents[ns] = s
...
if __name__ == "__main__":
    ...
    dest_state = State(500, 500)  # 0
    # dest_state = State(200, 600) # 1
    # dest_state = State(600, 80) # 2
